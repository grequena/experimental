...
A resposta para esta pergunta não é tão trivial, mas está na em analisar as variâncias. Considere o seguinte exemplo, para compreender o porque analisar variâncias é útil para comparar médias.
...
```{r, echo=FALSE}
# carregue o pacote tidyverse com library(tidyverse)
set.seed(2468)
n <- 10;a <- 4
trat <- 1:a |> rep(n) |> sort()
total <- rep(5,n*a)
...
d1 |> ggplot(aes(x=trat, y=y)) + geom_point()
d2 |> ggplot(aes(x=trat, y=w)) + geom_point()
```
